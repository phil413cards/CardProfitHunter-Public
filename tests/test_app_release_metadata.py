...
class AppReleaseMetadataTests(unittest.TestCase):
    ...
    def test_visible_version_comes_from_version_file(self) -> None:
        self.assertIn("APP_VERSION = load_display_version()", self.source)
        self.assertIn('page_title=f"Card Profit Hunter V{APP_VERSION}"', self.source)
        self.assertIn('st.title(f"Card Profit Hunter V{APP_VERSION} Professional Edition")', self.source)
    ...
    def test_roadmap_messages_do_not_claim_past_release_delivery(self) -> None:
        ...
        self.assertIn("planned for a future release", self.source)
